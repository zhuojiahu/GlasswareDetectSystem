	...
	: QWidget(parent)
{
	...
	for (int i = 0; i < pMainFrm->nCPUNumber; i++)
	{
...
void widget_info::slots_updateCameraFailureRate()
{
	...
	plotCameraFailureRate->populate();
	widgetPie->updatePie( pMainFrm->m_sRunningInfo.m_iErrorTypeCount,pMainFrm->m_sRunningInfo.m_checkedNum , pMainFrm->m_sRunningInfo.m_failureNumFromIOcard);
}
